...
# Import our modules
from models.transformer.dual_encoder import DualEncoderUNet
...
def train_dual_model(
    ...
    """Train the Dual Encoder U-Net model with CNN and transformer paths."""
    ...
    print(f"Training Dual Encoder U-Net with:")
    print(f"- Transformer: SwinUNETR (pretrained: {use_pretrained})")
    print(f"- CNN: Custom 3D CNN encoder")
    print(f"- Fusion: NFCE blocks")
    ...
    model, _ = DualEncoderUNet.load_model(best_model_path, device)
    model.eval()
    ...
    for metrics_dict in test_metrics:
        class_dices.append(metrics_dict['dice_per_class'])
        class_ious.append(metrics_dict['iou_per_class'])
    
    avg_class_dice = np.mean(class_dices, axis=0)
    avg_class_iou = np.mean(class_ious, axis=0)
    
    # Save test results
    results_path = os.path.join(experiment_dir, 'test_results.txt')
    with open(results_path, 'w') as f:
        f.write(f"Test Results for Dual Encoder U-Net:\n")
        f.write(f"Overall Dice Score: {test_dice:.4f}\n")
        f.write(f"Overall IoU Score: {test_iou:.4f}\n\n")
        
        f.write("Class-wise Results:\n")
        for i in range(num_classes):
            f.write(f"Class {i}:\n")
            f.write(f"  Dice Score: {avg_class_dice[i]:.4f}\n")
            f.write(f"  IoU Score: {avg_class_iou[i]:.4f}\n")
    ...
    for i in range(min(5, len(all_images))):  # Visualize up to 5 examples
        visualizer.plot_slices(
            ...
            save_path=os.path.join(vis_dir, f'test_prediction_{i+1}.png')
        )
        
        visualizer.create_3d_animation(
            all_images[i][0],
            all_predictions[i][0],
            save_path=os.path.join(vis_dir, f'test_prediction_{i+1}.gif')
        )
...
if __name__ == "__main__":
    ...
    # Train model
    model, metrics_logger = train_dual_model(
        device=device,
        epochs=100,
        batch_size=2,
        learning_rate=0.0001,
